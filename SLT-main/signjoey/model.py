...
def align_and_calculate_mpjpe(pred_tensor, ground_tensor):
    # 获取两个张量的帧数
    pred_frame = pred_tensor.size(1)
    true_frame = ground_tensor.size(1)

    # 找到较小的帧数
    min_frame = min(pred_frame, true_frame)

    pred_tensor_aligned = pred_tensor[:, :min_frame, :]
    true_tensor_aligned = ground_tensor[:, :min_frame, :]

    return pred_tensor_aligned, true_tensor_aligned
...
class SignModel(nn.Module):
    ...
    def run_batch(
        ...
        encoder_output_ground, encoder_hidden_ground = self.encode(
            sgn=batch_ground.sgn, sgn_mask=batch_ground.sgn_mask, sgn_length=batch_ground.sgn_lengths
        )
        ...
        sgn, sgn_ground = align_and_calculate_mpjpe(batch.sgn, batch_ground.sgn)

        return (decoded_gloss_sequences, stacked_txt_output, stacked_attention_scores,
                encoder_output, encoder_output_ground,
                sgn.view(sgn.shape[0], sgn.shape[1], 50, 3),
                sgn_ground.view(sgn_ground.shape[0], sgn_ground.shape[1], 50, 3))
