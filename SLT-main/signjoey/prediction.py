...
from signjoey.metrics import bleu, chrf, rouge, wer_list, mpjpe, fid, mpvpe, mpjae
...
def validate_on_data(
    ...
    valid_iter_ground = make_data_iter(
        dataset=ground_data,
        batch_size=batch_size,
        batch_type=batch_type,
        shuffle=False,
    ...
    with torch.no_grad():
        ...
        total_num_seqs = 0
        for i, (valid_batch, valid_batch_ground) in enumerate(zip(valid_iter, valid_iter_ground)):
            batch = Batch(
                is_train=False,
            ...
            batch_ground = Batch(
                is_train=False,
                torch_batch=valid_batch_ground,
                txt_pad_index=txt_pad_index,
                sgn_dim=sgn_dim,
                use_cuda=use_cuda,
                frame_subsampling_ratio=frame_subsampling_ratio,
            )
            sort_reverse_index,perm_index = batch.sort_by_sgn_lengths(None,None)
            sort_reverse_index, _ = batch_ground.sort_by_sgn_lengths(sort_reverse_index,perm_index)

            # Skip loss computation in test mode — saves an entire encoder forward pass
            if not skip_loss:
            ...
            if do_translation:
                ...
                all_encoder_outputs_ground.extend(batch_encoder_output_ground[sort_reverse_index].cpu())
                all_encoder_outputs.extend(batch_encoder_output[sort_reverse_index].cpu())
                all_sign.extend(batch_signs[sort_reverse_index].cpu())
                all_sign_ground.extend(batch_sign_grounds[sort_reverse_index].cpu())
        ...
        if do_recognition:
            ...
            if (
                not skip_loss
                and recognition_loss_function is not None
        ...
        if do_translation:
            ...
            if (
                not skip_loss
                and translation_loss_function is not None
            ...
            if dic['fid'] == None :
                # TXT Metrics
                dic['fid'] = fid(references=all_encoder_outputs_ground, hypotheses=all_encoder_outputs)
                dic['mpjpe'] = 0
                # dic['mpjpe'] = mpjpe(references=all_sign_ground, hypotheses=all_sign)
                dic['mpvpe'] = 0 #mpvpe(references=all_sign_ground, hypotheses=all_sign)
                dic['mpjae'] = 0
                # dic['mpjae'] = mpjae(references=all_sign_ground, hypotheses=all_sign)
    ...
    if do_translation:
        ...
        results["encoder_output"] = all_encoder_outputs
        results["decoder_output_ground"] = all_encoder_outputs_ground
...
def test(
    ...
    # load the data
    _, dev_data, test_data, gls_vocab, txt_vocab, ground_dev, ground_test = load_data(data_cfg=cfg["data"])
    ...
    logger.info(
        ...
        dic["fid"] if do_translation else -1,
        dic["mpjpe"] if do_translation else -1,
        dic["mpvpe"] if do_translation else -1,
        dic["mpjae"] if do_translation else -1,
    )
    logger.info("*" * 60)
    dic_test = {'fid':None,
         'mpjpe':None,
    ...
    logger.info(
        ...
        dic_test["fid"] if do_translation else -1,
        dic_test["mpjpe"] if do_translation else -1,
        dic_test["mpvpe"] if do_translation else -1,
        dic_test["mpjae"] if do_translation else -1,
    )
    logger.info("*" * 60)
